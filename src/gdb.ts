import { DebugSession, InitializedEvent, TerminatedEvent, ContinuedEvent, OutputEvent, Thread, ThreadEvent, StackFrame, Scope, Source, Handles, Event, Response } from 'vscode-debugadapter';
...
const COMMAND_MAP = (c: string) => c.startsWith('-') ? c.substring(1) : `interpreter-exec console "${c}"`;
...
export class GDBDebugSession extends DebugSession {
    ...
    private processLaunchAttachRequest(response: DebugProtocol.LaunchResponse, attach: boolean) {
        ...
        TcpPortScanner.findFreePorts(portFinderOpts, GDBServer.LOCALHOST).then((ports) => {
            ...
            if (this.args.gdbpath) {
                gdbExePath = path.normalize(this.args.gdbpath);
            }

            // Check to see if gdb exists.
            /*if (path.isAbsolute(gdbExePath) && gdbExePath=="") {
                if (fs.existsSync(gdbExePath) != false) {
                    this.sendErrorResponse(
                        response,
                        103,
                        `existsSync: ${this.serverController.name} GDB executable "${gdbExePath}" was not found.\n` +
                            'Please configure "module-debug.gdbpath" correctly. '
                    );
                    return;
                }
            }
                ...
                if (!hasbin.sync(gdbExePath.replace('.exe', ''))) {
                    this.sendErrorResponse(
                        response,
                        103,
                        `hasbin: ${this.serverController.name} GDB executable "${gdbExePath}" was not found.\n` +
                            'Please configure "module-debug.gdbpath" correctly.'
                    );
                    return;
                }
    ...
    protected customRequest(command: string, response: DebugProtocol.Response, args: any): void {
        ...
        switch (command) {
            ...
            case 'load-current-disassembly': // установка подсвеенной строки туда, где breakpoint
                this.getCurrentDisassembly().then(
                    (result) => {
    ...
    private getCurrentDisassembly(): Thenable<any> {
        return new Promise((resolve, reject) => {
            if ( this.currentThreadId > 0 ){
                this.miDebugger.sendCommand(`thread-info ${this.currentThreadId}`).then((result) => {
                    ...
                    const frame = MINode.valueOf(th, 'frame');
    ...
    protected changeRegisterRequest(response: DebugProtocol.Response, name: string, value: string){
        this.miDebugger.changeRegister(name, value).then((node) => {
            ...
            this.sendEvent(new TelemetryEvent('Error', 'Set register value', `name: ${name}; value: ${value}`));
        });
    }
        ...
        try {
            ...
            for (const variable of stack) {
                try {
                    ...
                    if (err instanceof MIError && err.message.search('unable to create variable object') >= 0) { 
                        variables.push({
                            name: variable.name,
                            value: `Undefined`,
