...
export class SymbolTable {
    ...
    public getFunctionByName(name: string, file?: string): SymbolInformation {
        ...
        let matches = this.symbols.filter((s) => s.type === SymbolType.Function && s.scope === SymbolScope.Local  && s.file === file);
        ...
        matches = this.symbols.filter((s) => s.type === SymbolType.Function && s.scope !== SymbolScope.Local);
        matches = matches.filter((m) => m.name.search(`${name}`) >= 0);
