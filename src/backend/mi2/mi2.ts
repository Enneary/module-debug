...
export class MI2 extends EventEmitter implements IBackend {
    ...
    public connect(cwd: string, executable: string, commands: string[]): Thenable<any> {
        ...
        return new Promise((resolve, reject) => {
            ...
            // const asyncPromise = this.sendCommand('target-async on', true);
...
// ответ на запрос
    ...
    public changeRegister(name: string, rawValue: string): Thenable<any> {
        if (trace) {
            this.log('stderr', 'changeVariable');
        }
        return this.sendCommand('gdb-set $' + name + '=' + rawValue);
    }
    ...
    public async varCreate(expression: string, name: string = '-', scope: string = '@'): Promise<VariableObject> {
        ...
        // "var-create watch_ca978112ca1bbdcafac231b39a23dc4da786eff8147c4e72b9807785afee48bb @ "a""
        // "var-create watch_ca978112ca1bbdcafac231b39a23dc4da786eff8147c4e72b9807785afee48bb @ "a""
    ...
    /*public async varFindInArgs(expression: string, name: string = '-', scope: string = '@'): Promise<VariableObject> {
        if (trace) {
            this.log('stderr', 'varFindInArgs');
        }
        let fmt = null;
        expression = expression.trim();
        if (/,[bdhonx]$/i.test(expression)) {
        ...
        const createResp = await this.sendCommand(`print ${expression}`);
        let overrideVal = null;
