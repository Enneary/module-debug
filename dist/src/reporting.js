...
const extension = vscode.extensions.getExtension('enneary.module-debug');
...
function getUUID() {
    if (!uuid) {
        const settingspath = path.join(os.homedir(), '.module-debug');
...
function telemetryEnabled() {
    ...
    const moduleDebug = vscode.workspace.getConfiguration('module-debug');
    return (telemetry.enableTelemetry && moduleDebug.enableTelemetry);
}
...
function beginSession(opts) {
    ...
    analytics.screenview('Debug Session', 'Module-Debug', extensionVersion, extensionId);
