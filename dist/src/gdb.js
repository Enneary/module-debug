...
const SERVER_TYPE_MAP = {
    ...
    external: external_1.ExternalServerController // используем для MC121.01
};
...
class GDBDebugSession extends vscode_debugadapter_1.DebugSession {
    ...
    launchRequest(response, args) {
        ...
        this.symbolTable = new symbols_1.SymbolTable(args.toolchainPath, args.toolchainPrefix, args.executable);
    ...
    processLaunchAttachRequest(response, attach) {
        ...
        tcpportscanner_1.TcpPortScanner.findFreePorts(portFinderOpts, server_1.GDBServer.LOCALHOST).then((ports) => {
            ...
            if (path.isAbsolute(gdbExePath)) {
                if (fs.existsSync(gdbExePath) === false) {
                    this.sendErrorResponse(response, 103, `${this.serverController.name} GDB executable "${gdbExePath}" was not found.\n` +
                        'Please configure "module-debug.armToolchainPath" correctly.');
                    return;
                }
            }
            else {
            ...
            this.server.init().then((started) => {
                ...
                this.initDebugger();
                // будет ли отображаться отладочная информация
                ...
                if (attach) {
                    ...
                    const preLaunchCommands = this.args.preLaunchCommands != null ?
                        this.args.preLaunchCommands.map(COMMAND_MAP) : [];
                    commands.push(...preLaunchCommands);
                    ...
                    const postLaunchCommands = this.args.postLaunchCommands != null ?
                        this.args.postLaunchCommands.map(COMMAND_MAP) : [];
                    commands.push(...postLaunchCommands);
                    commands.push(...this.serverController.swoCommands());
                }
                ...
                this.miDebugger.connect(this.args.cwd, this.args.executable, commands).then(() => {
                    this.handleMsg('log', 'miDebugger started \n');
                    this.started = true;
    ...
    // запрос к gdb на выполнение какой-нибудь команды
    customRequest(command, response, args) {
        ...
        switch (command) {
            ...
            case 'load-function-symbols':
                ...
                this.sendResponse(response);
                break;
            case 'load-current-disassembly': // установка подсвеенной строки туда, где breakpoint
                this.getCurrentDisassembly().then((result) => {
                    // this.activeEditorPath = result.file;
                    response.body = result;
                    this.sendResponse(response);
                }, (error) => {
                    response.body = error;
                    this.sendErrorResponse(response, 110, 'Unable to get current position');
                });
                break;
            /*case 'add-breakpoint':
            ...
            case 'read-register-list':
                ...
                break;
            case 'set-register':
                this.changeRegisterRequest(response, args['name'], args['value']);
                break;
            case 'disassemble':
            ...
            if (args.function) {
                try {
                    /* if ( this.args.servertype === 'external' ){
                        const result = await this.miDebugger.sendCommand('info line');
    ...
    getCurrentDisassembly() {
        return new Promise((resolve, reject) => {
            if (this.currentThreadId > 0) {
                this.miDebugger.sendCommand(`thread-info ${this.currentThreadId}`).then((result) => {
                    const tr = result.result('threads');
                    let th;
                    if (tr.length === 1) {
                        th = tr[0];
                    }
                    ...
                    const frame = mi_parse_1.MINode.valueOf(th, 'frame');
                    this.getDisassemblyForFunction(mi_parse_1.MINode.valueOf(frame, 'func'), mi_parse_1.MINode.valueOf(frame, 'file')).then((symbolInfo) => {
                        let line = -1;
                        let fname;
                        ...
                        const currentBreakpoints = (this.breakpointMap.get(url) || this.breakpointMap.get('disassembly:/' + fname) || []);
                        symbolInfo.instructions.forEach((inst, idx) => {
                            let prop = "none";
                            if (inst.address === mi_parse_1.MINode.valueOf(frame, 'addr')) {
                                line = idx + 1;
                                prop = "active";
                            }
                            else {
    ...
    changeRegisterRequest(response, name, value) {
        this.miDebugger.changeRegister(name, value).then((node) => {
            response.body = name;
            this.sendResponse(response);
        }, (error) => {
            response.body = { error: error };
            this.sendErrorResponse(response, 114, `Unable to set register value: ${error.toString()}`);
            this.sendEvent(new common_1.TelemetryEvent('Error', 'Set register value', `name: ${name}; value: ${value}`));
        });
    }
                ...
                for (const element of stack) {
                    ...
                    try {
                        ...
                        if (element.fileName && element.fileName.length > 0) {
                            ret.push(new vscode_debugadapter_1.StackFrame(stackId, `${element.function}`, new vscode_debugadapter_1.Source(element.fileName), element.line, 0));
                        }
                    }
                    catch (e) {
            ...
            try {
                ...
                for (const variable of stack) {
                    try {
                        ...
                        if (err instanceof backend_1.MIError && err.message.search('unable to create variable object') >= 0) {
                            variables.push({
                                name: variable.name,
                                value: `Undefined`,
