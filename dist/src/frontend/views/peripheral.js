...
class PeripheralTreeProvider {
            ...
            if (svdfile) {
                setTimeout(() => {
                    this.loadSVD(svdfile).then(() => {
                        vscode.workspace.findFiles('.vscode/.module-debug.peripherals.state.json', null, 1).then((value) => {
                            if (value.length > 0) {
    ...
    debugSessionTerminated() {
        if (vscode.workspace.workspaceFolders && vscode.workspace.workspaceFolders.length > 0) {
            const fspath = path.join(vscode.workspace.workspaceFolders[0].uri.fsPath, '.vscode', '.module-debug.peripherals.state.json');
