...
class RegisterTreeProvider {
    ...
    createRegisters(regInfo) {
        ...
        this.loaded = true;
        vscode_1.workspace.findFiles('.vscode/.module-debug.registers.state.json', null, 1).then((value) => {
            if (value.length > 0) {
    ...
    debugSessionTerminated() {
        if (vscode_1.workspace.workspaceFolders && vscode_1.workspace.workspaceFolders.length > 0) {
            const fspath = path.join(vscode_1.workspace.workspaceFolders[0].uri.fsPath, '.vscode', '.module-debug.registers.state.json');
